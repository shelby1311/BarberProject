    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-4 py-12">
      <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <div className="rounded-3xl border border-white/5 bg-zinc-900 p-8">
          {/* Toggle */}
          <div className="mb-6 grid grid-cols-2 gap-2 rounded-2xl bg-zinc-800 p-1">
            {(["client", "barber"] as const).map((r) => (
              <button key={r} type="button" onClick={() => setRole(r)}
                className={`flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold transition ${role === r ? "bg-amber-500 text-black shadow" : "text-zinc-400 hover:text-white"}`}>
                {r === "client" ? <User size={15} /> : <Store size={15} />}
                {r === "client" ? "Sou cliente" : "Sou barbeiro"}
              </button>
            ))}
          </div>
