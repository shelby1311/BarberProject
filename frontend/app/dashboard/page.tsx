...
import { Barber, Service, Appointment, WorkingHour, BlockedClient, BarberStatus } from "@/types";
...
export default function DashboardPage() {
  ...
  // ─── Status do Barbeiro ─────────────────────────────────────────────
  const [barberStatus, setBarberStatus] = useState<BarberStatus>("AVAILABLE");
  ...
  useEffect(() => {
    ...
    if (authBarber.barbershopId) {
      socket.emit("join:barbershop", authBarber.barbershopId);
    }
    ...
    socket.on("STATUS_CHANGE", (payload: { barberId: string; status: BarberStatus }) => {
      if (payload.barberId === authBarber.id) {
        setBarberStatus(payload.status);
      }
    });
    socket.on("QUEUE_UPDATE", (payload: { barberId: string; queueCount: number }) => {
      if (payload.barberId === authBarber.id) {
  ...
  // ─── Handlers de Status ─────────────────────────────────────────────
  const STATUS_OPTIONS: { value: BarberStatus; label: string; icon: React.ReactNode; color: string }[] = [
    { value: "AVAILABLE", label: "Disponível", icon: <Wifi size={14} />, color: "text-emerald-400 border-emerald-500/30 bg-emerald-500/10" },
    { value: "BUSY", label: "Ocupado", icon: <Briefcase size={14} />, color: "text-red-400 border-red-500/30 bg-red-500/10" },
    { value: "BREAK", label: "Pausa", icon: <Coffee size={14} />, color: "text-amber-400 border-amber-500/30 bg-amber-500/10" },
    { value: "OFFLINE", label: "Offline", icon: <WifiOff size={14} />, color: "text-zinc-400 border-zinc-500/30 bg-zinc-500/10" },
  ];

  async function handleStatusChange(status: BarberStatus) {
    setSavingStatus(true);
    try {
      await api.updateBarberStatus(status);
      setBarberStatus(status);
      setFeedback(`Status alterado para "${STATUS_OPTIONS.find((o) => o.value === status)?.label}"`);
      setTimeout(() => setFeedback(""), 3000);
    } finally {
  ...
  async function handleQueueAdd() {
    ...
    try {
      const result = await api.addQueue();
  ...
  async function handleQueueRemove() {
    setQueueLoading(true);
    try {
      const result = await api.removeQueue();
      setQueueCount(result.queueCount);
        ...
        <div className="grid gap-6">
          ...
          <section className="rounded-3xl border border-white/5 bg-zinc-900 p-4 sm:p-6">
            <h2 className="mb-4 flex items-center gap-2 font-bold text-white">
              <Wifi size={16} className="text-amber-500" /> Status
            </h2>
            <div className="flex flex-wrap gap-2">
              {STATUS_OPTIONS.map((opt) => {
                const isActive = barberStatus === opt.value;
                return (
                  <button
                    key={opt.value}
                    onClick={() => handleStatusChange(opt.value)}
                    disabled={savingStatus || isActive}
                    className={`flex items-center gap-1.5 rounded-xl border px-3 py-2 text-xs font-semibold transition-all active:scale-95 disabled:opacity-100 ${
                      isActive
                        ? `${opt.color} ring-1 ring-inset ring-current`
                        : "border-white/5 bg-zinc-800 text-zinc-500 hover:border-white/20 hover:text-white"
                    }`}
                  >
                    {opt.icon}
                    {opt.label}
          ...
          <section className="rounded-3xl border border-amber-500/20 bg-amber-950/10 p-4 sm:p-6">
            <h2 className="mb-1 flex items-center gap-2 font-bold text-white">
              <Users size={16} className="text-amber-500" /> Fila de Espera
            </h2>
            <p className="mb-4 text-xs text-zinc-500">Clientes aguardando atendimento presencial</p>
            <div className="flex items-center gap-6">
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black text-amber-400">{queueCount}</span>
                <span className="text-sm text-zinc-500">na fila</span>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={handleQueueAdd}
                  disabled={queueLoading}
                  className="flex items-center gap-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-3 py-2 text-xs font-semibold text-emerald-400 hover:bg-emerald-500/20 active:scale-95 transition-transform disabled:opacity-60"
                >
                  <UserPlus size={14} /> Adicionar
                </button>
                <button
                  onClick={handleQueueRemove}
                  disabled={queueLoading || queueCount <= 0}
                  className="flex items-center gap-1.5 rounded-xl bg-red-500/10 border border-red-500/20 px-3 py-2 text-xs font-semibold text-red-400 hover:bg-red-500/20 active:scale-95 transition-transform disabled:opacity-60"
                >
          ...
          {/* ─── Plano Atual ──────────────────────────────────────────── */}
          {data.barbershop && (
            <section className="rounded-3xl border border-amber-500/10 bg-gradient-to-br from-zinc-900 to-amber-950/20 p-4 sm:p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="flex items-center gap-2 font-bold text-white">
                    <Crown size={16} className="text-amber-500" /> Plano {data.barbershop.plan}
                  </h2>
                  <p className="mt-1 text-xs text-zinc-500">
                    {data.barbershop.plan === "ESSENTIAL" && "1 barbeiro · sem fila de espera"}
                    {data.barbershop.plan === "PRO" && "Até 5 barbeiros · fila de até 15 clientes · tempo real"}
                    {data.barbershop.plan === "ELITE" && "Até 15 barbeiros · fila ilimitada · tempo real"}
                  </p>
                </div>
                <a
                  href="/assinatura"
                  className="rounded-xl bg-amber-500/10 border border-amber-500/20 px-4 py-2 text-xs font-semibold text-amber-400 hover:bg-amber-500/20 transition"
                >
                  {data.barbershop.plan === "ESSENTIAL" ? "Fazer Upgrade" : "Gerenciar"}
                </a>
              </div>
            </section>
          )}
