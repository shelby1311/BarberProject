...
import { CalendarDays, Star, X, CheckCircle2, Clock, Scissors, AlertCircle } from "lucide-react";
...
export default function MinhaContaPage() {
  ...
  async function handleCancel(id: string) {
    ...
    try {
      ...
      setFeedback(e?.message ?? "Erro ao cancelar agendamento.");
  ...
  async function handleReview() {
    ...
    try {
      ...
      const e = err as { message?: string };
      setFeedback(e?.message ?? "Erro ao enviar avaliação.");
      setTimeout(() => setFeedback(""), 4000);
    } finally {
      setSubmitting(false);
    }
          ...
          {feedback && (
            <motion.div
              ...
              className={`mb-6 flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium ${
                feedback.includes("Erro") || feedback.includes("cancelar")
                  ? "border-red-500/20 bg-red-950/40 text-red-400"
                  : "border-emerald-500/20 bg-emerald-950/40 text-emerald-400"
              }`}
            >
              {feedback.includes("Erro") ? <AlertCircle size={15} /> : <CheckCircle2 size={15} />}
              {feedback}
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {displayed.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {displayed.map((a) => {
              ...
              const canCancel = ["pending", "confirmed"].includes(a.status) && date > now;
