...
import { MapPin, Link2, Clock, Star, X, ChevronLeft, ChevronRight, Share2, MessageCircle, CalendarCheck, Wifi, WifiOff, Coffee, Briefcase } from "lucide-react";
        ...
        <div className="mb-8">
          ...
          {barber.status && (
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <LiveStatusBadge status={barber.status} />
              {typeof barber.queueCount === 'number' && barber.queueCount > 0 && (
                <span className="inline-flex items-center gap-1 rounded-full border border-amber-500/20 bg-amber-500/10 px-2.5 py-0.5 text-xs font-semibold text-amber-400">
                  <Clock size={11} />
                  {barber.queueCount} na fila
                </span>
              )}
              {barber.currentServiceEstimatedEnd && (
                <span className="inline-flex items-center gap-1 rounded-full border border-zinc-500/20 bg-zinc-500/10 px-2.5 py-0.5 text-xs font-semibold text-zinc-400">
                  <Clock size={11} />
                  Livre ~{new Date(barber.currentServiceEstimatedEnd).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}
                </span>
              )}
            </div>
          )}
...
// ─── Componente de Badge de Status ao Vivo ─────────────────────────────
function LiveStatusBadge({ status }: { status: BarberStatus }) {
  const config: Record<BarberStatus, { label: string; icon: React.ReactNode; color: string; ping: string }> = {
    AVAILABLE: {
      label: 'Disponível',
      icon: <Wifi size={11} />,
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-semibold ${cfg.color}`}>
      <span className="relative flex h-2 w-2">
        <span className={`absolute inline-flex h-full w-full animate-ping rounded-full opacity-75 ${cfg.ping}`} />
        <span className={`relative inline-flex h-2 w-2 rounded-full ${cfg.ping}`} />
      </span>
      {cfg.icon}
      {cfg.label}
    </span>
