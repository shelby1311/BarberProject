...
/**
 * Buffer de limpeza após o término estimado de um serviço (5 minutos).
 * Usado no cálculo de ETC (Estimated Time Completion).
 */
const ETC_CLEANUP_BUFFER_MINUTES = 5;
...
/**
 * Calcula o ETC (Estimated Time Completion) para um serviço em andamento.
 *
 * Regra:
 *   estimatedEnd = currentServiceStartedAt + durationMinutes + ETC_CLEANUP_BUFFER_MINUTES
 *
 * Se o barbeiro não estiver ocupado (status !== "BUSY") ou não tiver
 * currentServiceStartedAt, retorna null.
 *
 * @param startedAt - Data/hora de início do serviço atual
 * @param durationMinutes - Duração em minutos do serviço sendo realizado
 * @param status - Status atual do barbeiro
 * @returns Data estimada de término ou null
 */
export function calculateETC(
  ...
  if (status !== "BUSY" || !startedAt || !durationMinutes) {
    return null;
  }

  const estimatedEnd = new Date(startedAt.getTime() + durationMinutes * 60 * 1000);
  estimatedEnd.setMinutes(estimatedEnd.getMinutes() + ETC_CLEANUP_BUFFER_MINUTES);
  return estimatedEnd;
}
