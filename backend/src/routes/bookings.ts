 ...
 *         $ref: '#/components/schemas/ErrorResponse'
 */
bookingRouter.post("/", validate(BookingSchema), async (req, res, next) => {
  try {
    const parsed = req.body as z.infer<typeof BookingSchema>;
...
/**
 * @openapi
 * /api/bookings/{barberId}/slots:
 *   get:
 *     summary: Slots disponíveis para uma data
 *     tags: [Bookings]
 *     parameters:
 *       - in: path
 *         name: barberId
 ...
 *     responses:
 *       200:
 *         description: Lista de slots ISO
 *         content:
 *           application/json:
 *             schema:
 *               type: array
...
bookingRouter.patch("/:id/cancel", authMiddleware, async (req: AuthRequest, res, next) => {
  try {
    ...
    logger.info({ appointmentId: req.params.id, role: req.role }, "Agendamento cancelado");
